package com.example.helpers;

import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.Locale;

import com.j256.ormlite.field.DatabaseField;

/**
 * A simple demonstration object we are creating and persisting to the database.
 */
public class EmployeeDao {

	// id is generated by the database and set on the object automagically
	@DatabaseField(generatedId = true)
	int id;
	@DatabaseField(index = true) //new
	String tcID;
	@DatabaseField  //new 
	String firstname;
	@DatabaseField //new 
	String lastname;
	@DatabaseField
	long millis;
	@DatabaseField
	Date date;
	@DatabaseField
	Date birthDate;
	@DatabaseField
	boolean even;

	EmployeeDao() {
		// needed by ormlite
	}

	public EmployeeDao(long millis,String tcno,String fname, String lname,Date birthDate) {
		
		this.date = new Date(millis);
		this.tcID=tcno;
		this.firstname=fname;
		this.lastname=lname;
		this.birthDate=birthDate;
		this.millis = millis;
		this.even = ((millis % 2) == 0);
	}

	@Override
	public String toString() {
		StringBuilder sb = new StringBuilder();
		sb.append("id=").append(id);
		sb.append("tcID=").append(tcID);
		sb.append(", ").append("firstname=").append(firstname);
		sb.append(", ").append("lastname=").append(lastname);
		SimpleDateFormat dateFormatterDMY = new SimpleDateFormat("dd MMMM yyyy", new Locale("tr"));
		sb.append(", ").append("birthDate=").append(dateFormatterDMY.format(birthDate));
		sb.append(", ").append("ms=").append(millis);
		SimpleDateFormat dateFormatter = new SimpleDateFormat("MM/dd HH:mm:ss", Locale.US);
		sb.append(", ").append("date=").append(dateFormatter.format(date));
		sb.append(", ").append("even=").append(even);
		return sb.toString();
	}
}
